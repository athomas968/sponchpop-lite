...
class planet_disk_evo:      # PLANET FORMATION & DISK EVOLUTION CLASS WITHOUT CHEMISTRY
    """
...
class planet_disk_chem:     # PLANET FORMATION & DISK EVOLUTION CLASS WITH CHEMISTRY
    """
    A class that represents a planet. It simulates the evolution of the planet by updating its attributes at each time step.
    ...
    def __init__(self, m_0, a_0, t_0, B, p_ratio, dt_yrs, PDM, migration, peb_acc, plan_acc, gas_acc, grain_growth, pebble_drift, disk):
        ...
        self.dust_surface_density           = self.disk.get_Sigma(radius = self.r_grid_centres , time = 0.0, status=1) * (self.disk.dustr * self.disk.dtgr)
        self.pebble_sigma_evo               = np.zeros((len(self.t_range), len(self.r_grid_centres)))
        self.stokes_evo                     = np.zeros((len(self.t_range), len(self.r_grid_centres)))  # stokes number evolution across disk at each grid centre
        
        # chem specific additions
        self.species_evo                    = np.zeros([len(self.t_range), len(self.pebble_surface_density), 3])       
        species_coldens = {
            'H2S':          (self.gas_surface_density * 1e-5 * (chem.m_H2S / (chem.m_H2))).copy(),
            'FeS_dust':     np.zeros_like(self.gas_surface_density),
            'Fe_dust':      ((self.dust_surface_density + self.pebble_surface_density) * 0.2).copy()}
            # 'Fe_dust':      np.zeros_like(self.gas_surface_density)}
        self.species_evo[0, :, 0]           = species_coldens['H2S']
        self.species_evo[0, :, 1]           = species_coldens['FeS_dust']
        self.species_evo[0, :, 2]           = species_coldens['Fe_dust']
    ...
    def single_planet_case(self):
        ...
        for i, t in enumerate(self.t_range):
            ...
            if self.t_range[i] <= self.t_0*ds.yr:
                self.species_evo[i, :, 0]           = species_coldens['H2S']
                self.species_evo[i, :, 1]           = species_coldens['FeS_dust']
                self.species_evo[i, :, 2]           = species_coldens['Fe_dust']
                self.key_vals[i,:]                  = 0
                ...
                self.stokes_evo[i, :] = self.st.copy()
                ...
                if self.pebble_drift == True:
                    ...
                    twodee_out[-1]                      = 0
                    delta_sigma                         = (-twodee_in + twodee_out) / self.grid_sizes
                    # store old pebble surface density before drift
                    old_pebble_density                  = self.pebble_surface_density.copy()
                    self.pebble_surface_density        += delta_sigma * self.dt
                    # scale Fe and FeS proportionally to pebble drift (they're embedded in pebbles)
                    pebble_ratio                        = np.where(old_pebble_density > 0, self.pebble_surface_density / old_pebble_density, 1.0)
                    species_coldens['FeS_dust']        *= pebble_ratio
                    species_coldens['Fe_dust']         *= pebble_ratio
                elif self.pebble_drift == False:
                    radial_drift                        = np.zeros_like(self.r_grid_centres)
                    prev_gas_surface_density            = self.disk.get_Sigma(radius = self.r_grid_centres , time = self.t_range[i-1], status=1).flatten()
                    self.pebble_surface_density        *= self.gas_surface_density / prev_gas_surface_density
                ...
                species_coldens = chem.propagate_chem_microchem_S(
                    self.disk, self.gas_surface_density, dust_and_pebbles, self.t_range[i], self.dt,
                    dtime=self.dt/5, species_coldens=species_coldens,
                ...
                if self.pebble_drift == True:
                    ...
                    twodee_out                          = np.roll(twodee_in, -1)
                    twodee_out[-1]                      = 0
                    # capture pebble density BEFORE any changes for accretion removal calculation
                    pre_pebble_sigma                    = self.pebble_surface_density[bindex]
                    self.pebble_surface_density[bindex] = max(0, (self.pebble_surface_density[bindex] - pebble_deduct))
                    delta_sigma                         = (-twodee_in + twodee_out) / self.r_grid_centres
                    old_pebble_density                  = self.pebble_surface_density.copy()
                    self.pebble_surface_density        += delta_sigma * self.dt
                ...
                species_coldens = chem.propagate_chem_microchem_S(
                    self.disk, self.gas_surface_density, dust_and_pebbles, self.t_range[i], self.dt,
                    dtime=self.dt/5, species_coldens=species_coldens,
                    accretion_removal=[(bindex, pebble_deduct, pre_pebble_sigma)])

                self.species_evo[i, :, 0]           = species_coldens['H2S'].flatten()
                self.species_evo[i, :, 1]           = species_coldens['FeS_dust'].flatten()
                self.species_evo[i, :, 2]           = species_coldens['Fe_dust'].flatten()
    ...
    def get_vals(self):
        return self.key_vals, self.t_range, self.stokes_evo
...
class disk_chem:            # DISK EVOLUTION CLASS WITH CHEMISTRY
    ...
    Attributes:
        ...
        disk (object):              disk model
        t_f (float):                final time for the simulation, yrs
    ...
    def __init__(self, t_f, dt_yrs, grain_growth, pebble_drift, disk):
        self.grain_growth   = grain_growth
        self.pebble_drift   = pebble_drift

        self.disk           = disk                                                          # disk model       
        self.t_f            = t_f                                                           # yrs, birth time of planet
        self.dt_yrs         = dt_yrs                                                        # yrs, time step of simulation
        
        self.t_final        = self.t_f                                                      # yrs, final time of simulation
        self.dt             = self.dt_yrs * ds.yr                                           # s, time step of simulation
        self.t_range        = np.arange(float(0.0), self.t_final, self.dt_yrs)*ds.yr        # array of time values for full disk evolution and planet formation
        ...
        # chem specific additions
        self.species_evo                    = np.zeros([len(self.t_range), len(self.pebble_surface_density), 3])
        self.velocities                     = np.zeros([len(self.t_range), 3])  # array to store dadt and radial drift velocities at each time step
    
    def evo_disk_case(self):
        """
        Simulates the evolution of a single planet in a disk, updating its attributes at each time step.
        """
        self.st                 = self.disk.get_st_ini(time=0.0, radius=self.r_grid_centres, status=1)
        ...
        species_coldens = {
            'H2S':          (self.gas_surface_density * 1e-5 * (chem.m_H2S / (chem.m_H2))).copy(),
            'FeS_dust':     np.zeros_like(self.gas_surface_density),
            'Fe_dust':      (dust_and_pebbles * 0.2).copy()}
        ...
        for i, t in enumerate(self.t_range):
            ...
            validation           = self.disk.validate(radius=10*ds.auSI, time=self.t_range[i])
            if validation != 1:
                print('Radius/ time out of bounds')
                break

            if self.t_range[i] <= self.t_f*ds.yr:
                if i == 0:
                    species_coldens = {
                        'H2S':          (self.gas_surface_density * 1e-5 * (chem.m_H2S / (chem.m_H2))).copy(),
                        'FeS_dust':     np.zeros_like(self.gas_surface_density),
                    ...
                    # chem drift for species
                    for key in ['FeS_dust', 'Fe_dust']:
                        twodee_in_species = species_coldens[key] * radial_drift
                        twodee_out_species = np.roll(twodee_in_species, -1)
                        twodee_out_species[-1] = 0
                        delta_sigma_species = (-twodee_in_species + twodee_out_species) / self.grid_sizes
                        species_coldens[key] += delta_sigma_species * self.dt
                        species_coldens[key] = np.maximum(species_coldens[key], 0)    
    ...
    def get_other(self):
        return self.pebble_surface_density, self.planetesimal_grid, self.pebble_sigma_evo, self.velocities, self.species_evo
